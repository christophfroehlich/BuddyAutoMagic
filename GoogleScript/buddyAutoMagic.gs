...
function onOpen() {  
  ...
  var menuEntries = [ 
    ...
    {name: "A. Check if mailaccount has quota left to send mails", functionName: "dailyQuotaLeft"}
  ];
...
function copyResponses() {
  var local = SpreadsheetApp.openById(LOCAL_ID),
      ...
      localR = local.getSheetByName("Form responses").getDataRange().getValues(),
      internationalR = international.getSheetByName("Form responses").getDataRange().getValues(),
...
function calcAffinities() {
 ...
 //  maxAssignments = 2; // comment upper part and uncomment this for debugging: Browser.inputBox disturbes debugger
  ...
  for (var i = 0; i < locals.length; i++) {
   ...
   if (as.length == 0) {
    Browser.msgBox("There are no locals for assignment left!");
    return false;
  }
  ...
  for (var i = 0; i < internationals.length; i++) {
   ...
   if (bs.length == 0) {
    Browser.msgBox("There are no incomings for assignment left!");
    return false;
  }
...
function sendOutAssignmentEmailsDebug() {
  ...
  var email = "b.wuerkner@gmail.com";
...
//  Logger.log("local: %s \nexchange: %s", localMessage.htmlBody ,exchangeMessage.htmlBody);
  ...
  for (var i = 0; i < assignments.length; i++) {
    ...
    if (!a.emailSent) { // Email not already sent
      ...
      var keys = [];
      for(var key in a) Logger.log(key);
...
//      Logger.log(localBody);
      ...
      sendEmail({  
        ...
        noReply: true,
        replyTo: "office@buddynetwork.at"
      });
