...
// affinity |?'f?n?ti| noun ( pl. affinities )
...
//  Logger.log("Calculating affinity(%s, %s)", A.firstName, B.firstName);
  for (var i = 0; i < questions.length; i++) {
    ...
    // just for debugging: does affinity calulation work for interests-question?
//    if (question.name == "Interests") {
//      var importance = question.importance(qi);
//      var affinityQuestion = question.question(A, B, qi);
    ...
    question: function (A, B) {
      ...
      if (!gotPreferredCountry(A, B)) {
        ...
        return Math.max(
          ...
          0.5*countryPreference(A.pref2, B.exchangeHomeCountry),
          0.25*countryPreference(A.pref3, B.exchangeHomeCountry));
      }
    },
    isBad: function (A, B) {
...
function arrivaltimeQuestion() {
  return {
    ...
    question: function(A, B) {
      ...
      if (daysFromNow(date) < 7) { // one weeks from now
        return 2;
      } else if (daysFromNow(date) < 14) { // two weeks from now
        return 1;
      } else {
        return 0;
      }
...
function capacity(response) {
  switch (response) {
    ...
    case "Yes, I love being a buddy!":
      return 3;
    case "Nein, ich freu mich auf genau einen":
      return 1;
    case "Ja, aber nicht mehr als zwei":
      return 2;
    case "Ja, ich bin ur-gern Buddy!":
      return 3;
    default:
